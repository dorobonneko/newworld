...
public class Indexdm530 extends Index
{
	...
	public String getIndex(int page)
	{
		...
		try
		{
			...
			while(tabs_i.hasNext()){
				...
				item.put("href",makeUrl(a.attr("href")));
			}
			...
			if (mains != null)
			{
				...
				while (mains_i.hasNext())
				{
					...
					main.put("href", makeUrl(main_item.selectFirst("a").attr("href")));
	...
	public String getList(String url)
	{
		...
		try
		{
			...
			Document doc=Jsoup.connect(url.replace("index1.html","index.html")).userAgent("Mozilla (Linux;Android 10)").get();
...
private String getPage(String url){
	...
	public String makeUrl(String url)
	{
		switch(url){
			...
			case "/bmjuchang/":
			case "/bmzhenren/":
				return getHost()+url.substring(1)+"%d.html";
			case "/topic/2019.html":
			case "/top/new.html":
			case "/top/topic.html":
			...
			return url+"index%d.html";
			}{
				if(url.endsWith("index.html"))
					return getHost()+ url.substring(1).replace("index.html","index%d.html");
				return getHost()+ url.substring(1)+"index%d.html";
			}
			//return getHost()+url.substring(1)+"index%d.html";
		}
